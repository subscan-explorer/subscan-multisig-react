...
const chainStandard =
  'background: radial-gradient(circle, rgba(255,255,255,1) 0%, rgba(42,170,244,1) 35%, rgba(10,10,10,1) 100%)';
...
export const chainColors = Object.entries({
  ...
  'ZERO.IO': chainZero,
}).reduce((colors, [chain, color]) => ({ ...colors, [sanitize(chain)]: color }), {}); // Alphabetical overrides based on the actual software node type
...
}).reduce((colors, [node, color]) => ({ ...colors, [sanitize(node)]: color }), {}); // Alphabetical overrides based on the actual software node type
// NOTE: This is as retrieved via the system.name RPC
...
export const specColors = Object.entries({
  ...
  statemint: specStatemint,
  westmint: specWestmint,
}).reduce((colors, [spec, color]) => ({ ...colors, [sanitize(spec)]: color }), {});
